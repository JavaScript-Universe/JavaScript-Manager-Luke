...
module.exports = {
  ...
  run: async (client, msg, args, prefix, command, Discord, MessageEmbed) => {
    ...
    if (!args[0])
      return msg.reply(
        "Please use the command like this: !addinfo <prefix> <canned message>"
      );
    const cannedName = args[0];
    if (!args.join(" ").replace(args[0], "").includes("`"))
      return msg.reply(
        "Please put the canned message in a single backtick codeblock `like this`"
      );
    let cannedMsg = args
      .join(" ")
      .replace(args[0], "")
    ...
    if (new Enmap({ name: "cannedMsgs" }).has(cannedName))
      return msg.reply(
        `There already is a canned message with the prefix \`${cannedName}\`. Please delete it first using \`!delcanned <prefix>\``
      );
    new Enmap({ name: "cannedMsgs" }).set(cannedName, cannedMsg);
    return msg.reply(
      `You successfully added a canned message with the prefix \`${cannedName}\``
    );
  },
};
