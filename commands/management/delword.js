...
module.exports = {
  ...
  run: async (client, msg, args, prefix, command, Discord, MessageEmbed) => {
    ...
    let word = args[0];
    if (!word)
      return msg.reply("Please use the command like this: !delword <word>");
    word = word.toLowerCase();
    if (!new Enmap({ name: "badWords" }).has(word))
      return msg.reply(`The bad word \`${word}\` doesn't exist!`);
    new Enmap({ name: "badWords" }).delete(word);
    return msg.reply(
      `You successfully removed the bad word \`${word}\` from the list`
    );
  },
};
