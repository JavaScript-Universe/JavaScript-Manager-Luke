...
module.exports = {
  ...
  run: async (client, msg, args, prefix, command, Discord, MessageEmbed) => {
    ...
    if (!toWarn) return msg.reply('Please insert a user to bean!').then(d => d.delete({ timeout: 5000 })).then(msg.delete({ timeout: 2000 }));
    warnsDB.ensure(toWarn.id, {warns: {}});
    let reason = args.join(' ').replace(args[0], '').trim();
    if (!reason) return msg.reply('Please insert the reason you want to bean this user for!').then(d => d.delete({ timeout: 5000 })).then(msg.delete({ timeout: 2000 }));
    ...
    .setColor("ORANGE")
    .setTimestamp();
    return await msg.channel.send(emChan).then(d => d.delete({ timeout: 6000 })).then(msg.delete({ timeout: 0 }));
  }
}