...
module.exports = {
  ...
  run: async (client, msg, args, prefix, command, Discord, MessageEmbed) => {
    ...
    if (!toWarn)
      return msg
        .reply("Please insert a member to kick!")
        .then((d) => d.delete({ timeout: 5000 }))
        .then(msg.delete({ timeout: 2000 }));
    ...
    if (!reason)
      return msg
        .reply("Please insert the reason you want to kick this member for!")
        .then((d) => d.delete({ timeout: 5000 }))
        .then(msg.delete({ timeout: 2000 }));
    if (cannedMsgs.has(reason)) reason = cannedMsgs.get(reason);
    if (moderator.id == toWarn.id)
      return msg.reply("You may not kick yourself dumby!");
    if (
      ...
      return msg
        .reply(
          "You may not kick someone with the same rank or a rank higher as yourself!"
        )
        .then((d) => d.delete({ timeout: 5000 }))
        .then(msg.delete({ timeout: 2000 }));
    ...
    const emUser = new MessageEmbed()
      ...
      .setColor("ORANGE")
      .setDescription(
        `You were kicked from **JavaScript Universe** for ${reason}, please don't do it again!`
      )
