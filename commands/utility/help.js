...
module.exports = {
  ...
  run: async (
    ...
    const embed = new MessageEmbed()
      .setColor(color)
      .setFooter(msg.author.username, msg.author.avatarURL()); // Create embed so you dont create it 9000 times.
    ...
    if (query) {
      ...
      commands.forEach((cmd) => {
        // Fetch all commands and descriptions.
        output.push(
          `\`${prefix}${cmd.name}\` - ${cmd.description || "Not specified"}`
        );
      });
      ...
      if (commands.length <= 0) {
        try {
          ...
          if (client.commands.has(command) || client.aliases.has(command)) {
            ...
            embed.addField(
              ...
              command.cooldown
                ? moment
                    ...
                    .format("D [days], H [hrs], m [mins] [and] s [secs]")
                : "None"
            );
            embed.addField(
              `» Aliases`,
            ...
            // Command doesnt exist in either aliases or commands.
            embed.setTitle("Something went wrong!");
            embed.setColor("RED");
            embed.setDescription(
              `It seems **${query}** is not a valid category, or a command name`
            );
            msg.channel.send(embed);
          }
        } catch (_) {
        ...
        if (
          premcategories.includes(query.toLowerCase()) &&
          !settings(msg.guild.id).premium.enabled
        ) {
          embed.setTitle(`Help » ${query}`);
          embed.setDescription(
            `Please purchase premium on [patreon](https://www.patreon.com/GCAPremium)`
          );
          msg.channel.send(embed);
    ...
    if (!query) {
      ...
      embed
        .setTitle("Help")
        .setDescription(
          `Please select a category to see its available commands.\nUsage: \`${prefix}help <command/category>\``
        )
        .addField(
