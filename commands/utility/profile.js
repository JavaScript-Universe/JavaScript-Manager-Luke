  ...
  run: async (client, msg, args, prefix, command, Discord, MessageEmbed) => {
    ...
    if (member.id == msg.author.id) {
      ...
      if (staff) {
        ...
        em.addField(
          "**• (Week) Messages Count (aprox.)**",
          db.staffChecks.get(msg.author.id).count.toLocaleString()
        );
      }
      em.addField(
        "**• JavaScript Knowledge**",
        db.users.get(msg.author.id).knowledge || "None"
      );
      em.addField(
        "**• Biography**",
        db.users.get(msg.author.id).bio || "No bio set up yet"
      );
      if (canPromote(msg.author.id)) {
        em.addField(
          "**• Projects**",
          db.users.get(msg.author.id).projects || "None"
        );
      }
      ...
      if (
        client.guilds.cache
          ...
          ? client.guilds.cache
              .get("812011009682178089")
              .members.cache.get(member.id)
              .roles.cache.has("812011009682178091")
          : false
      ) {
        em.addField(
          "**• Rank**",
        ...
        em.addField(
          "**• (Week) Messages Count (aprox.)**",
          db.staffChecks.get(member.id).count.toLocaleString()
        );
      }
      em.addField(
        "**• JavaScript Knowledge**",
        db.users.get(member.id).knowledge || "None"
      );
      em.addField(
        "**• Biography**",
        db.users.get(member.id).bio || "No bio set up yet"
      );
      if (canPromote(member.id)) {
        em.addField(
          "**• Projects**",
          db.users.get(member.id).projects || "None"
        );
      }
