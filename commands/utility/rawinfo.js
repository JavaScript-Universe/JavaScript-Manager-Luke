    ...
    if (!args[0])
      return msg.channel.send(
        new MessageEmbed()
          ...
          .setDescription(
            `\`${
              new Enmap({ name: "infos" }).keyArray().length != 0
                ? new Enmap({ name: "infos" }).keyArray().join(", ")
                : "no tags available"
            }\``
          )
      );
    ...
    if (!new Enmap({ name: "infos" }).has(infoName))
      return msg.channel.send(
        new MessageEmbed()
          ...
          .setColor(color)
          .setDescription(
            `I could not find info with the tag **${infoName}**. Please run this command without arguments to get a full list of all available tags.`
          )
      );
