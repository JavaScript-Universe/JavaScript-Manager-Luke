...
module.exports = async (client, messageReaction) => {
  ...
  if (messageReaction.count < 5) {
    ...
    const matchingContent =
      starboard.messages.cache.find(
        (m) =>
          m.embeds[0].footer.text.split(" : ")[0] == messageReaction.message.id
      ) || false;
    ...
    if (!matchingContent) {
      // Create starboard message here.
      const em = new MessageEmbed()
        .setAuthor(
          messageReaction.message.author.username,
      ...
      em.addField(
        ...
        "⭐".repeat(messageReaction.count)
      );
      em.addField(
        "Link to message",
        `[Click Me](https://discord.com/channels/720661480143454340/${messageReaction.message.channel.id}/${messageReaction.message.id})`
      );
      if (messageReaction.message.attachments.size > 0) {
        em.setImage(messageReaction.message.attachments.first().url);
      }
      em.setFooter(
        `${messageReaction.message.id} : ${messageReaction.message.channel.id}`
      );
      return starboard.send(em).catch((err) => console.error(err));
    }
