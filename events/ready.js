...
module.exports = async (client) => {
  ...
  console.log(
    `${client.user.tag}, ready in ${client.guilds.cache.size} servers.`,
    "ready"
  );
  ...
  starboard.messages.cache
    .filter((m) => !m.deleted && m.embeds.size > 0)
    .forEach(async (mesg) => {
    ...
    setInterval(async () => {
      if (Date.now() - lastPost > WEEK) {
        ...
        for (const data of filter) {
          em.addField(
            `${(await client.users.fetch(data[1].id)).tag} | ${
              (await client.guilds.cache
                .get("720661480143454340")
                .members.fetch(data[1].id))
                ? (
                    await client.guilds.cache
                      .get("720661480143454340")
                      .members.fetch(data[1].id)
            ...
            `${data[1].count.toLocaleString()} messages sent`
          );
        }
        checkChan.send(em);
        db.clear();
  ...
  // Mutes system
  setInterval(() => {
    const mutedDB = new Enmap({ name: "mutes" }).forEach(async (db) => {
      ...
      await server
        .member(db.id)
        .send("You were unmuted in **JavaScript Universe**");
