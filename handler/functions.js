...
async function warn(msg, id, points, reason) {
  ...
  const em = new MessageEmbed()
    ...
    .addField(
      "Total Points",
      `\`${parseInt(warnedTotalPoints).toLocaleString()}\``
    )
    .addField(
      "Other Cases",
      `\`${
        Object.keys(warnsDB.get(id).warns).length != 0
          ? Object.keys(warnsDB.get(id).warns).join("\n")
          : "none"
      }\``
    )
  ...
  warnsDB.set(
    id,
    { moderator: "auto mod", points: parseInt(points), reason: reason },
    `warns.${caseID}`
  );
  if (warnsDB.get(id).points == 4) {
    ...
    await msg.guild.members.ban(id, {
      reason: `Moderator: Auto Mod, Reason: ${reason}`,
    });
  }
  return true;
}
